...
"""
Benchmark M3.2 Root system: steady state small root system solved with PlantHydraulicModel
...
# RSML geometry for benchmark M3.2 (was RootSystem.rsml in older tutorials)
rsml_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "RootSystem14.rsml")
...
r = HydraulicModel_Meunier(rsml_file, param, cached=False)  # or HydraulicModel_Doussan
r.test()
...
""" Numerical solution (a): constant conductivities """
...
""" Numerical solution (b): age-dependent conductivities per root type """
...
param.set_kr_age_dependent(kr0[:, 0].tolist(), kr0[:, 1].tolist(), subType=[0, 1])
param.set_kr_age_dependent(kr1[:, 0].tolist(), kr1[:, 1].tolist(), subType=[2, 3])
...
param.set_kx_age_dependent(kx0[:, 0].tolist(), kx0[:, 1].tolist(), subType=[0, 1])
param.set_kx_age_dependent(kx1[:, 0].tolist(), kx1[:, 1].tolist(), subType=[2, 3])
...
# """ Additional vtk plot """
# radial_fluxes = r.radial_fluxes(simtime, rx_b, [p_s], cells=True)
# axial_fluxes = r.axial_fluxes(simtime, rx_b, [p_s], cells=True)
#
# ana = pb.SegmentAnalyser(r.ms)
# ana.addData("rx", rx_b)
# ana.addData("radial", radial_fluxes)
# ana.addData("axial", axial_fluxes)
# pd = vp.segs_to_polydata(ana, 1., ["radius", "subType", "creationTime", "length", "rx", "axial", "radial"])
# vp.plot_roots(pd, "radial")
